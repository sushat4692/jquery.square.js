...
(function($) {
    ...
    var Square = function($target, opt) {
        ...
        self.target      = self.parentObj.find("*[class^="+self.set.target+"]").css({position:"absolute"});
    ...
    Square.prototype.setPosition = function(sp) {
        ...
        self.target.each( function(i) {
            ...
            var pos = {
                x:self.space+self.size*mpos.x+
                  self.space*mpos.x,
                y:self.space+self.size*mpos.y+
                  self.space*mpos.y
            };
            if( sp > 0 ) {
            ...
            tHeight = pos.y+sizey+self.space > tHeight ?
